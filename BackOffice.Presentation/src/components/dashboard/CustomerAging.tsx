...
const CustomerAging: React.FC<Props> = ({ filters }) => {
  const [ref, isVisible] = useLazyLoad();

  const { data, isLoading, error, refetch } = useDashboardApi<AgingData>(
    () => dashboardService.getCustomerAging(filters.storeId),
    [filters.storeId],
    isVisible
  );
  ...
  return (
    <div ref={ref} className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-sm dash-fade-in">
      <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-4">Customer Aging</h3>

      {!isVisible || isLoading ? (
        <AgingSkeleton />
      ) : error || !data ? (
        <CardErrorState message={error || 'Failed to load customer aging'} onRetry={refetch} />
      ) : (
        <>
          ...
          <div className="border-t border-gray-100 dark:border-gray-700 mt-4 pt-3 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm font-bold text-gray-900 dark:text-white">{data.customerCount ?? 0}</div>
            </div>
            <div className="text-right">
              <span className="text-xs text-gray-500 dark:text-gray-400">Total Receivable</span>
