...
// Record interface matching ItemOnPhoneOrderGridDto
interface ItemOnPhoneOrderRecord {
  qty: number | null
  name: string | null
...
const normalColumnDefs: GridColDef[] = [
  {
    field: "name",
    headerName: "Name",
    width: 220,
...
// Aggregated columns (no Store column)
const aggregatedColumnDefs: GridColDef[] = normalColumnDefs.filter(
  (col) => col.field !== "storeName"
)

// Grid ID for settings persistence
const ITEMS_ON_PHONE_ORDER_GRID_ID = "items-on-phone-order-list-grid"

const ItemOnPhoneOrderListPage = memo(function ItemOnPhoneOrderListPage() {
  ...
  // Include All Stores
  const [includeAllStores, setIncludeAllStores] = useState(false)
  ...
  // Convert column definitions based on aggregate mode
  const defaultColumns = useMemo(
    () => convertToGridColumns(aggregated ? aggregatedColumnDefs : normalColumnDefs),
    [aggregated]
  )
  ...
  } = useGridSettings(ITEMS_ON_PHONE_ORDER_GRID_ID, defaultColumns)
  ...
  const additionalParams = useMemo(() => {
    ...
    // Filter by current store unless Include All Stores is checked
    if (!includeAllStores && currentStore?.storeId) {
    ...
    if (debouncedSearchText.trim()) {
      ...
      params.CustomGridSearchColumns =
        "name,modalNumber,barcodeNumber,phoneOrderType,storeName"
    }
  ...
  const handleRowUpdate = useCallback(
    async (updatedRow: ItemOnPhoneOrderRecord) => {
      // View item details on double-click
  ...
  const handleRowSelection = useCallback((itemStoreID: string) => {
    setSelectedRows((prevSelectedRows) => {
      ...
      if (newSelectedRows.has(itemStoreID)) {
  ...
  const handleClearFilters = useCallback(() => {
    ...
    setPhoneStatus("")
    setAggregated(false)
    setIncludeAllStores(false)
    setSearchText("")
    setDebouncedSearchText("")
    showToast("Filters cleared", "info")
  }, [showToast])

  // Handle aggregate toggle - remount grid with new columns
  const handleAggregateToggle = useCallback(() => {
    setAggregated((prev) => !prev)
    setRemountKey((prev) => prev + 1)
  }, [])
  ...
  const fetchAllData = useCallback(async (dateFrom?: string, dateTo?: string): Promise<any[]> => {
    try {
      ...
      const response = await axios({
        method: "GET",
        url: API_ENDPOINTS.ITEMS_ON_PHONE_ORDER.GET_ALL,
        params: {
          startRow: 0,
          endRow: 1000000,
          ...(dateFrom && { dateFrom }),
  ...
  const exportModal = useExportModal({
    columns,
    fetchAllData,
    filename: "item-on-phone-order",
    pdfOptions: { title: "Item On Phone Order", orientation: "landscape" },
  ...
  return (
    <div
      className="items-on-phone-order-list-page p-2 mx-auto md:p-2 min-h-full"
      style={{
        height: "100%",
        display: "flex",
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-2">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {(phoneStatus || aggregated || scope !== "all" || searchText || includeAllStores) && (
        ...
        {filterPanelExpanded && (
          <div className="px-4 pb-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-wrap items-center gap-4 py-3 border-b border-gray-100 dark:border-gray-700">
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    {currentStore?.storeName || "No Store Selected"}
                  </span>
                </div>
              </div>

              {/* Include All Stores Checkbox */}
              <label className="flex items-center gap-2 cursor-pointer select-none mt-4">
                <input
                  type="checkbox"
                  checked={includeAllStores}
                  onChange={() => setIncludeAllStores(!includeAllStores)}
              ...
              {/* Aggregate Items Checkbox */}
              <label className="flex items-center gap-2 cursor-pointer select-none mt-4 ml-4 pl-4 border-l border-gray-300 dark:border-gray-600">
                <input
                  type="checkbox"
                  checked={aggregated}
      ...
      <div style={{ flex: 1, minHeight: 0, height: "100%" }}>
        <ServerGrid
          key={`items-on-phone-order-grid-${remountKey}`}
          data={[]}
          columns={columns}
          loading={false}
          error={null}
          ...
          emptyIcon="📋"
          serverSide={true}
          apiUrl={API_ENDPOINTS.ITEMS_ON_PHONE_ORDER.GET_ALL}
          methodType="GET"
          ...
          showCheckboxes={true}
          getRowId={(row) => row.itemStoreID || `${row.name}-${row.barcodeNumber}`}
...
export default ItemOnPhoneOrderListPage
