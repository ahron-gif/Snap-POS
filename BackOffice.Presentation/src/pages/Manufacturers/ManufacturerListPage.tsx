...
const manufacturerColumnDefs: GridColDef[] = [
  {
...
const MANUFACTURERS_GRID_ID = "manufacturers-list-grid"

const ManufacturerListPage = memo(function ManufacturerListPage() {
  ...
  // WEB-187: side-panel state for opening Manufacturer edit form on row click.
  // Mirrors ItemGroupListPage's pattern for consistency across Inventory screens.
  ...
  const defaultColumns = useMemo(() => convertToGridColumns(manufacturerColumnDefs), [])
  ...
  } = useGridSettings(MANUFACTURERS_GRID_ID, defaultColumns)
  ...
  const { filteredColumns: columns } = useColumnAccessFilter(MANUFACTURERS_GRID_ID, userPrefColumns)
  ...
  const additionalParams = useMemo(() => {
    ...
    return {
      ...
      CustomGridSearchColumns: "manufacturerName,manufacturerNo",
    }
  }, [debouncedSearchText])
  ...
  const handleRowSelection = useCallback((manufacturerID: string) => {
    setSelectedRows((prevSelectedRows) => {
      const newSelectedRows = new Set(prevSelectedRows)
      if (newSelectedRows.has(manufacturerID)) {
        newSelectedRows.delete(manufacturerID)
      } else {
        newSelectedRows.add(manufacturerID)
      }
  ...
  const handleBulkDelete = useCallback(async () => {
    if (selectedRows.size === 0) {
      showToast("Please select manufacturers to delete", "info")
      return
    }

    const confirmed = await confirm({
      title: 'Delete Manufacturers',
      message: `Are you sure you want to delete ${selectedRows.size} manufacturer(s)?`,
      variant: 'danger',
    })
    if (!confirmed) return
    ...
    if (successCount > 0) {
      showToast(`${successCount} manufacturer(s) deleted successfully`, "success")
      setSelectedRows(new Set())
      setRemountKey((prev) => prev + 1)
    }
    if (failCount > 0) {
      showToast(`${failCount} manufacturer(s) could not be deleted`, "error")
    }
  ...
  const handleBulkExport = useCallback(() => {
    if (selectedRows.size > 0) {
      showToast(`Exporting ${selectedRows.size} manufacturers...`, "info")
    }
  ...
  const handleStaticEdit = useCallback(() => {
    if (selectedRows.size === 1) {
      ...
      showToast("Please select only one manufacturer to edit", "info")
    } else {
      showToast("Please select a manufacturer to edit", "info")
    }
  }, [selectedRows, openTab, showToast])

  const handleStaticDownloadReport = useCallback(() => {
    showToast("Downloading report for manufacturers", "success")
  }, [showToast])
  ...
  const handleSelectAll = useCallback(() => {
    if (serverGridSelectAllRef.current) {
      ...
      showToast("Selecting all manufacturers...", "info")
    }
  }, [showToast])

  // Handle row double-click to edit
  const handleRowUpdate = useCallback(
    async (updatedRow: ManufacturerRecord) => {
      openTab({
        component: "ManufacturerFormPage",
        title: "Edit Manufacturer",
        closable: true,
        props: { id: updatedRow.manufacturerID },
      })
    },
    [openTab]
  )
  ...
  const handleViewAction = useCallback(
    (row: ManufacturerRecord) => {
      openTab({
        component: "ManufacturerFormPage",
        title: "View Manufacturer",
        closable: true,
        props: { id: row.manufacturerID, viewMode: true },
      })
    },
    [openTab]
  )
  ...
  // (matches Item Groups). Previously this called openTab.
  const handleEditAction = useCallback(
    (row: ManufacturerRecord) => {
      setSidePanel({ open: true, id: row.manufacturerID, isNew: false })
    },
    []
  )
  ...
  const handleDeleteAction = useCallback(
    async (row: ManufacturerRecord) => {
      const confirmed = await confirm({
        title: 'Delete Manufacturer',
        message: `Are you sure you want to delete manufacturer "${row.manufacturerName}"?`,
        variant: 'danger',
      })
      if (!confirmed) return

      try {
        const canDelete = await manufacturerService.canDeleteManufacturer(row.manufacturerID)
        if (!canDelete.success || !canDelete.data) {
          showToast("This manufacturer cannot be deleted. It may be in use.", "error")
          return
        }

        const result = await manufacturerService.deleteManufacturer(row.manufacturerID)
        if (result.success) {
          showToast(`Manufacturer "${row.manufacturerName}" deleted successfully`, "success")
          setRemountKey((prev) => prev + 1)
        } else {
          showToast("Failed to delete manufacturer", "error")
        }
      } catch {
        showToast("An error occurred while deleting the manufacturer", "error")
      }
    },
    [showToast, confirm]
  )
  ...
  const fetchAllData = useCallback(async (dateFrom?: string, dateTo?: string): Promise<any[]> => {
    try {
      ...
      const response = await axios({
        method: "GET",
        url: API_ENDPOINTS.MANUFACTURERS.GET_ALL_MANUFACTURERS,
        params: {
          startRow: 0,
          endRow: 1000000,
          ...(dateFrom && { dateFrom }),
  ...
  const {
    ...
    gridDataRef,
    fetchAllData,
    filename: "manufacturers-list",
    pdfOptions: { title: "Manufacturers List", orientation: "landscape" },
  })
  ...
  const auditHistoryContextMenuItem: CustomContextMenuItem = useMemo(() => ({
    ...
    onClick: (row: any) => {
      const record = row as ManufacturerRecord
      setAuditEntityId(record.manufacturerID)
      setAuditEntityName(record.manufacturerName || "")
  ...
  return (
    <div
      className="manufacturers-list-page p-2 mx-auto md:p-2 min-h-full"
      style={{
        height: "100%",
        display: "flex",
    ...
    >
      ...
      <ActionHeader
        ...
        gridId={MANUFACTURERS_GRID_ID}
      />

      {/* Main Grid Component */}
      <div style={{ flex: 1, minHeight: 0 }}>
        <ServerGrid
          key={`manufacturers-grid-${remountKey}`}
          data={[]}
          columns={columns}
          gridId={MANUFACTURERS_GRID_ID}
          ...
          emptyIcon="🏭"
          serverSide={true}
          apiUrl={API_ENDPOINTS.MANUFACTURERS.GET_ALL_MANUFACTURERS}
          methodType="GET"
          getAuthHeaders={memoizedGetAuthHeaders}
          defaultSortColumn="manufacturerName"
          containerWidth="47%"
          ...
          customContextMenuItems={[auditHistoryContextMenuItem]}
        />
      </div>
