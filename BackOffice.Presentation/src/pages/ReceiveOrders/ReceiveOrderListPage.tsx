...
// Receive Order record interface based on ReceiveOrderGridDto
interface ReceiveOrderRecord {
  receiveID: string
  ...
  isDiscAmount: boolean | null
  discountSum: number | null
  entriesSum: number | null
  receiveOrderDate: string | null
  status: number | null
...
// Column definitions for receive orders - matching the original VB.NET screen columns
const receiveOrderColumnDefs: GridColDef[] = [
  {
    field: "billDate",
...
const RECEIVE_ORDERS_GRID_ID = "receive-orders-list-grid"

const ReceiveOrderListPage = memo(function ReceiveOrderListPage() {
  ...
  // Open Bills filter (default checked like the original)
  ...
  const defaultColumns = useMemo(
    () => convertToGridColumns(receiveOrderColumnDefs),
    []
  )
  ...
  } = useGridSettings(RECEIVE_ORDERS_GRID_ID, defaultColumns)
  ...
  const additionalParams = useMemo(() => {
    ...
    if (debouncedSearchText.trim()) {
      ...
      params.CustomGridSearchColumns =
        "billNo,supplierName,supplierCode,storeName,userName,packingSlipNo,poNo,accountNo,contactName,note"
    }
  ...
  const handleRowUpdate = useCallback(
    async (updatedRow: ReceiveOrderRecord) => {
      openTab({
        component: "ReceiveOrderFormPage",
        title: `Edit: Receive ${updatedRow.billNo || ""}`,
        closable: true,
        props: { id: updatedRow.receiveID },
      })
    },
    [openTab]
  )

  // Handle opening the add receive order page
  const handleAddReceiveOrder = useCallback(() => {
    openTab({
      component: "ReceiveOrderFormPage",
  ...
  // Handle checkbox selection using receiveID as the primary identifier
  const handleRowSelection = useCallback((receiveID: string) => {
    setSelectedRows((prevSelectedRows) => {
      const newSelectedRows = new Set(prevSelectedRows)
      const wasSelected = newSelectedRows.has(receiveID)

      if (wasSelected) {
        newSelectedRows.delete(receiveID)
      } else {
        newSelectedRows.add(receiveID)
      }
  ...
  const handleBulkDelete = useCallback(async () => {
    if (selectedRows.size > 0) {
      const confirmed = await confirm({
        title: 'Delete Receive Orders',
        message: `Are you sure you want to delete ${selectedRows.size} receive order(s)? This action cannot be undone.`,
        variant: 'danger',
      })
      if (confirmed) {
        console.log("Bulk deleting receive orders:", selectedRows)
        setSelectedRows(new Set())
        showToast(
          `${selectedRows.size} receive orders deleted successfully!`,
          "success"
        )
      }
    }
  ...
  const handleBulkExport = useCallback(() => {
    if (selectedRows.size > 0) {
      console.log("Bulk exporting receive orders:", selectedRows)
      showToast(`Exporting ${selectedRows.size} receive orders...`, "info")
    }
  }, [selectedRows, showToast])

  // Handle View Details from context menu
  const handleViewReceiveOrder = useCallback(
    (row: ReceiveOrderRecord) => {
      openTab({
        component: "ReceiveOrderFormPage",
        title: `View: Receive ${row.billNo || ""}`,
        closable: true,
        props: { id: row.receiveID, readOnly: true },
      })
    },
    [openTab]
  )
  ...
  const handleEditReceiveOrder = useCallback(
    (row: ReceiveOrderRecord) => {
      openTab({
        component: "ReceiveOrderFormPage",
        title: `Edit: Receive ${row.billNo || ""}`,
        closable: true,
        props: { id: row.receiveID },
      })
    },
    [openTab]
  )
  ...
  const handleDeleteReceiveOrder = useCallback(
    async (row: ReceiveOrderRecord) => {
      const confirmed = await confirm({
        title: 'Delete Receive Order',
        message: `Are you sure you want to delete receive order ${row.billNo || row.receiveID}? This action cannot be undone.`,
        variant: 'danger',
      })
      if (confirmed) {
        showToast(
          `Receive order ${row.billNo || row.receiveID} deleted`,
          "success"
        )
        handleRemountGrid()
      }
  ...
  const handleVoidReceiveOrder = useCallback(
    async (row: ReceiveOrderRecord) => {
      const confirmed = await confirm({
        title: 'Void Receive Order',
        message: `Are you sure you want to void receive order ${row.billNo || row.receiveID}?`,
        variant: 'warning',
      })
      if (confirmed) {
        showToast(
          `Receive order ${row.billNo || row.receiveID} voided`,
          "success"
        )
        handleRemountGrid()
      }
    ...
    () => [
      {
        ...
        onClick: handleViewReceiveOrder,
      },
      {
        label: "New",
        icon: (
      ...
      },
      {
        ...
        onClick: handleEditReceiveOrder,
      },
      {
        label: "Void",
        icon: (
        ...
        onClick: handleVoidReceiveOrder,
        color: "#d97706",
        hoverBgColor: "#fffbeb",
        dividerBefore: true,
      },
      {
        ...
        onClick: handleDeleteReceiveOrder,
        color: "#dc2626",
        hoverBgColor: "#fef2f2",
      },
    ],
    [
      handleViewReceiveOrder,
      handleAddReceiveOrder,
      handleEditReceiveOrder,
      handleVoidReceiveOrder,
      handleDeleteReceiveOrder,
    ]
  )
  ...
  const fetchAllData = useCallback(async (dateFrom?: string, dateTo?: string): Promise<any[]> => {
    try {
      ...
      const response = await axios({
        method: "GET",
        url: API_ENDPOINTS.RECEIVE_ORDERS.GET_ALL,
        params: {
          startRow: 0,
          endRow: 1000000,
          ...(dateFrom && { dateFrom }),
          ...(dateTo && { dateTo }),
          sortColumn: "receiveOrderDate",
          sortDirection: "desc",
          ...additionalParams,
        },
        headers,
  ...
  const exportModal = useExportModal({
    ...
    fetchAllData,
    filename: "receive-orders-list",
    pdfOptions: { title: "Receive Orders List", orientation: "landscape" },
  ...
  const handleSelectAll = useCallback(() => {
    try {
      if (serverGridSelectAllRef.current) {
        ...
        showToast("Selecting all receive orders...", "info")
      }
    } catch (error) {
      console.error("Error in handleSelectAll:", error)
      showToast("Error selecting receive orders", "error")
    }
  }, [showToast])

  return (
    <div
      className="receive-orders-list-page p-2 mx-auto md:p-2 min-h-full"
      style={{
        height: "100%",
        display: "flex",
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-2">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {(openBills || showVoid || scope !== "all" || searchText) && (
            ...
            <div className="flex flex-wrap items-center gap-4 py-3 border-b border-gray-100 dark:border-gray-700">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input
                  type="checkbox"
                  checked={openBills}
                  onChange={() => setOpenBills(!openBills)}
      ...
      <ActionHeader
        ...
        itemType="receive orders"
        onAddNew={handleAddReceiveOrder}
      ...
      <div style={{ flex: 1, minHeight: 0, height: "100%" }}>
        <ServerGrid
          key={`receive-orders-grid-${remountKey}`}
          data={[]}
          columns={columns}
          loading={false}
          error={null}
          ...
          columnChooser={true}
          title="Receive Orders List"
          emptyMessage="No receive orders found"
          emptyIcon="📦"
          serverSide={true}
          apiUrl={API_ENDPOINTS.RECEIVE_ORDERS.GET_ALL}
          methodType="GET"
          getAuthHeaders={memoizedGetAuthHeaders}
          defaultSortColumn="receiveOrderDate"
          containerWidth="47%"
