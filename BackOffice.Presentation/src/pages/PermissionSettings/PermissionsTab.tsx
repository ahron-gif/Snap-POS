...
import { fetchPermissions, createPermission, updatePermission, deletePermission, setSelectedPermission } from '../../store/slices/permissionSlice';
import type { Permission, CreatePermissionDto, UpdatePermissionDto } from '../../services/smartKartReg/permissionApi';
...
const PermissionsTab: React.FC = () => {
  ...
  const { permissions, totalRecords, loading, saving, error } = useSelector((state: RootState) => state.permission);
  ...
  const loadPermissions = useCallback(() => {
    ...
    if (search.trim()) {
      ...
      params.customGridSearchColumns = 'PermissionKey,PermissionName,Description,Category';
    }
  ...
  const openCreateModal = () => {
    ...
    setFormData({ permissionKey: '', permissionName: '', description: '', category: '', isActive: true });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPermission) {
        const dto: UpdatePermissionDto = { ...formData, id: editingPermission.id };
        await dispatch(updatePermission({ id: editingPermission.id, dto })).unwrap();
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-gray-500 dark:text-gray-400">
                    <div className="flex flex-col items-center gap-2">
                      <svg className="w-12 h-12 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                ...
                permissions.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-750">
                    <td className="px-4 py-3 font-mono text-xs text-gray-800 dark:text-gray-200">{p.permissionKey}</td>
                    <td className="px-4 py-3 text-gray-800 dark:text-gray-200">{p.permissionName}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${p.isActive ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
                        {p.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex justify-center gap-2">
                        <button onClick={() => openEditModal(p)} className="text-brand-500 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300" title="Edit">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                        </button>
                        <button onClick={() => setShowDeleteConfirm(p.id)} className="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" title="Delete">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                ...
                <input type="text" required value={formData.permissionKey} onChange={(e) => setFormData({ ...formData, permissionKey: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-brand-500" placeholder="e.g. ViewItems" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Permission Name *</label>
                <input type="text" required value={formData.permissionName} onChange={(e) => setFormData({ ...formData, permissionName: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-brand-500" placeholder="e.g. View Items" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Category</label>
                <input type="text" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-brand-500" placeholder="e.g. Items" />
              </div>
              <div>
      ...
      {showDeleteConfirm !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-sm p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Delete Permission</h3>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-5">Are you sure you want to delete this permission? This action cannot be undone.</p>
