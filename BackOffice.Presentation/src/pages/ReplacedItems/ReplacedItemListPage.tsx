...
// Record interface matching ReplacedItemGridDto
interface ReplacedItemRecord {
  ...
  phoneOrderStatus: string
  oldQty: number
  newQty: number
  userCollected: string
  removedItem: string | null
  removedModelNo: string | null
  removedUPC: string | null
  addedItem: string | null
  addedModelNo: string | null
  addedUPC: string | null
  action: string
}
...
const columnDefs: GridColDef[] = [
  ...
  {
    field: "phoneOrderStatus",
    headerName: "Phone Order Status",
    width: 140,
    type: "string",
    sortable: true,
    filterable: true,
  ...
  {
    field: "removedModelNo",
    headerName: "Removed Model No",
    width: 140,
    type: "string",
    sortable: true,
    filterable: true,
...
const REPLACED_ITEMS_GRID_ID = "replaced-items-list-grid"

const ReplacedItemListPage = memo(function ReplacedItemListPage() {
  ...
  // Date filter state - default 10 years back matching VB.NET
  ...
  } = useGridSettings(REPLACED_ITEMS_GRID_ID, defaultColumns)
  ...
  const handleScopeChange = useCallback((newScope: string) => {
    ...
    switch (newScope) {
      ...
      case "all":
      default:
        setFromDate(new Date(Date.now() - 10 * 365 * 24 * 60 * 60 * 1000).toISOString().split("T")[0])
        setToDate(new Date(Date.now() + 1 * 24 * 60 * 60 * 1000).toISOString().split("T")[0])
        return
    }
  ...
  const additionalParams = useMemo(() => {
    ...
    if (debouncedSearchText.trim()) {
      ...
      params.CustomGridSearchColumns =
        "phoneOrderNo,customerNo,phone,cell,lastName,firstName,userCollected,removedItem,removedModelNo,addedItem,addedModelNo,action"
    }

    // Date filters passed directly as query params (SP takes fromDate/toDate)
    if (fromDate) {
    ...
    return params
  }, [debouncedSearchText, fromDate, toDate])
  ...
  const handleRowUpdate = useCallback(
    async (_updatedRow: ReplacedItemRecord) => {
      // No specific action on double-click for replaced items
    },
    []
  )
  ...
  // Context menu - View Phone Order
  const handleViewPhoneOrder = useCallback(
    (row: ReplacedItemRecord) => {
      if (row.phoneOrderNo) {
        openTab({
          component: "PhoneOrderFormPage",
          title: `View: Phone Order ${row.phoneOrderNo}`,
          closable: true,
          props: { phoneOrderNo: row.phoneOrderNo, readOnly: true },
        })
      }
    },
    [openTab]
  ...
  const fetchAllData = useCallback(async (dateFrom?: string, dateTo?: string): Promise<any[]> => {
    try {
      ...
      const response = await axios({
        method: "GET",
        url: API_ENDPOINTS.REPLACED_ITEMS.GET_ALL,
        params: {
          startRow: 0,
          endRow: 1000000,
          ...(dateFrom && { dateFrom }),
  ...
  const exportModal = useExportModal({
    ...
    fetchAllData,
    filename: "replaced-items",
    pdfOptions: { title: "Replaced Items", orientation: "landscape" },
  ...
  return (
    <div
      className="replaced-items-list-page p-2 mx-auto md:p-2 min-h-full"
      style={{
        height: "100%",
        display: "flex",
    ...
    >
      ...
      <ActionHeader
        ...
        onRefresh={() => { showToast("Refreshing grid...", "info"); setTimeout(handleRemountGrid, 300) }}
      ...
      <div style={{ flex: 1, minHeight: 0, height: "100%" }}>
        <ServerGrid
          key={`replaced-items-grid-${remountKey}`}
          data={[]}
          columns={columns}
          loading={false}
          error={null}
          ...
          columnChooser={true}
          title="Replaced Items"
          emptyMessage="No replaced items found"
          emptyIcon="🔄"
          serverSide={true}
          apiUrl={API_ENDPOINTS.REPLACED_ITEMS.GET_ALL}
          methodType="GET"
          ...
          onPageNavigation={(callbacks) => { pageNavigationRef.current = callbacks }}
          showCheckboxes={true}
...
export default ReplacedItemListPage
