...
// Receive Transfer record interface based on ReceiveTransferGridDto
interface ReceiveTransferRecord {
  receiveTransferID: string
  receiveDate: string | null
  transferID: string | null
  transferNo: string | null
  reciveNo: string | null
  transferStatus: number | null
  transferDate: string | null
  note: string | null
  transferUser: string | null
  receiveUser: string | null
  storeReceived: string | null
...
// Column definitions matching the Transfers Received screen from the old app
const receiveTransferColumnDefs: GridColDef[] = [
  {
    field: "receiveDate",
...
const RECEIVE_TRANSFERS_GRID_ID = "receive-transfers-list-grid"

const ReceiveTransferListPage = memo(function ReceiveTransferListPage() {
  ...
  const defaultColumns = useMemo(
    () => convertToGridColumns(receiveTransferColumnDefs),
    []
  )
  ...
  } = useGridSettings(RECEIVE_TRANSFERS_GRID_ID, defaultColumns)
  ...
  const additionalParams = useMemo(() => {
    ...
    if (debouncedSearchText.trim()) {
      ...
      params.CustomGridSearchColumns =
        "transferNo,reciveNo,fromStore,toStore,storeReceived,note,transferUser,receiveUser"
    }
    ...
    // Date range filters on ReceiveDate
  ...
  const handleRowUpdate = useCallback(
    async (updatedRow: ReceiveTransferRecord) => {
      console.log("View receive transfer:", updatedRow.receiveTransferID)
    },
    []
  )
  ...
  const handleRowSelection = useCallback((receiveTransferID: string) => {
    setSelectedRows((prevSelectedRows) => {
      const newSelectedRows = new Set(prevSelectedRows)
      const wasSelected = newSelectedRows.has(receiveTransferID)

      if (wasSelected) {
        newSelectedRows.delete(receiveTransferID)
      } else {
        newSelectedRows.add(receiveTransferID)
      }
  ...
  const handleBulkDelete = useCallback(async () => {
    if (selectedRows.size > 0) {
      const confirmed = await confirm({
        title: 'Delete Receive Transfers',
        message: `Are you sure you want to delete ${selectedRows.size} receive transfer(s)? This action cannot be undone.`,
        variant: 'danger',
      })
      if (confirmed) {
        console.log("Bulk deleting receive transfers:", selectedRows)
        setSelectedRows(new Set())
        showToast(
          `${selectedRows.size} receive transfers deleted successfully!`,
          "success"
        )
      }
    }
  ...
  const handleBulkExport = useCallback(() => {
    if (selectedRows.size > 0) {
      console.log("Bulk exporting receive transfers:", selectedRows)
      showToast(`Exporting ${selectedRows.size} receive transfers...`, "info")
    }
  }, [selectedRows, showToast])

  // Handle View Details from context menu
  const handleViewReceiveTransfer = useCallback(
    (row: ReceiveTransferRecord) => {
      console.log("View receive transfer:", row.receiveTransferID)
    },
    []
  )
    ...
    () => [
      {
        ...
        onClick: handleViewReceiveTransfer,
      },
    ],
    [handleViewReceiveTransfer]
  )
  ...
  const fetchAllData = useCallback(async (dateFrom?: string, dateTo?: string): Promise<any[]> => {
    try {
      ...
      const response = await axios({
        method: "GET",
        url: API_ENDPOINTS.RECEIVE_TRANSFERS.GET_ALL,
        params: {
          startRow: 0,
          endRow: 1000000,
          ...(dateFrom && { dateFrom }),
  ...
  const {
    ...
    gridDataRef,
    fetchAllData,
    filename: "receive-transfers-list",
    pdfOptions: {
  ...
  const exportModal = useExportModal({
    ...
    fetchAllData,
    filename: "receive-transfers-list",
    pdfOptions: { title: "Transfers Received List", orientation: "landscape" },
  ...
  const handleSelectAll = useCallback(() => {
    try {
      if (serverGridSelectAllRef.current) {
        ...
        showToast("Selecting all receive transfers...", "info")
      }
    } catch (error) {
      console.error("Error in handleSelectAll:", error)
      showToast("Error selecting receive transfers", "error")
    }
  }, [showToast])

  return (
    <div
      className="receive-transfers-list-page p-2 mx-auto md:p-2 min-h-full"
      style={{
        height: "100%",
        display: "flex",
    ...
    >
      ...
      <div style={{ flex: 1, minHeight: 0, height: "100%" }}>
        <ServerGrid
          key={`receive-transfers-grid-${remountKey}`}
          data={[]}
          columns={columns}
          loading={false}
          error={null}
          ...
          columnChooser={true}
          title="Transfers Received List"
          emptyMessage="No receive transfers found"
          emptyIcon="📥"
          serverSide={true}
          apiUrl={API_ENDPOINTS.RECEIVE_TRANSFERS.GET_ALL}
          methodType="GET"
          getAuthHeaders={memoizedGetAuthHeaders}
          defaultSortColumn="receiveDate"
          containerWidth="47%"
          ...
          showCheckboxes={true}
          getRowId={(row) => row.receiveTransferID}
...
export default ReceiveTransferListPage
