...
// Transfer record interface based on TransferGridDto
interface TransferRecord {
  transferID: string
  transferNo: string | null
  transferDate: string | null
  totalCost: number | null
  transferStatusDec: string | null
  note: string | null
...
// Column definitions matching the Transfers screen
const transferColumnDefs: GridColDef[] = [
  {
    field: "transferNo",
...
const TRANSFERS_GRID_ID = "transfers-list-grid"

const TransferListPage = memo(function TransferListPage() {
  ...
  const defaultColumns = useMemo(
    () => convertToGridColumns(transferColumnDefs),
    []
  )
  ...
  } = useGridSettings(TRANSFERS_GRID_ID, defaultColumns)
  ...
  const additionalParams = useMemo(() => {
    ...
    if (debouncedSearchText.trim()) {
      ...
      params.CustomGridSearchColumns =
        "transferNo,to_Store,from_Store,transferStatusDec,note,userName"
    }
    ...
    // Date range filters on TransferDate
  ...
  const handleRowUpdate = useCallback(
    async (updatedRow: TransferRecord) => {
      console.log("View transfer:", updatedRow.transferID)
    },
    []
  )
  ...
  const handleRowSelection = useCallback((transferID: string) => {
    setSelectedRows((prevSelectedRows) => {
      const newSelectedRows = new Set(prevSelectedRows)
      const wasSelected = newSelectedRows.has(transferID)

      if (wasSelected) {
        newSelectedRows.delete(transferID)
      } else {
        newSelectedRows.add(transferID)
      }
  ...
  const handleBulkDelete = useCallback(async () => {
    if (selectedRows.size > 0) {
      const confirmed = await confirm({
        ...
        message: `Are you sure you want to delete ${selectedRows.size} transfer(s)? This action cannot be undone.`,
        variant: 'danger',
      })
      if (confirmed) {
        console.log("Bulk deleting transfers:", selectedRows)
        setSelectedRows(new Set())
        showToast(
          `${selectedRows.size} transfers deleted successfully!`,
          "success"
        )
      }
    }
  ...
  const handleBulkExport = useCallback(() => {
    if (selectedRows.size > 0) {
      console.log("Bulk exporting transfers:", selectedRows)
      showToast(`Exporting ${selectedRows.size} transfers...`, "info")
    }
  ...
  const handleViewTransfer = useCallback(
    (row: TransferRecord) => {
      console.log("View transfer:", row.transferID)
    },
    []
  )
  ...
  const fetchAllData = useCallback(async (dateFrom?: string, dateTo?: string): Promise<any[]> => {
    try {
      ...
      const response = await axios({
        method: "GET",
        url: API_ENDPOINTS.TRANSFERS.GET_ALL,
        params: {
          startRow: 0,
          endRow: 1000000,
          ...(dateFrom && { dateFrom }),
          ...(dateTo && { dateTo }),
          sortColumn: "transferDate",
          sortDirection: "desc",
          ...additionalParams,
        },
        headers,
  ...
  const exportModal = useExportModal({
    ...
    fetchAllData,
    filename: "transfers-list",
    pdfOptions: { title: "Transfers List", orientation: "landscape" },
  ...
  const handleSelectAll = useCallback(() => {
    try {
      if (serverGridSelectAllRef.current) {
        ...
        showToast("Selecting all transfers...", "info")
      }
    } catch (error) {
      console.error("Error in handleSelectAll:", error)
      showToast("Error selecting transfers", "error")
    }
  }, [showToast])

  return (
    <div
      className="transfers-list-page p-2 mx-auto md:p-2 min-h-full"
      style={{
        height: "100%",
        display: "flex",
    ...
    >
      ...
      <div style={{ flex: 1, minHeight: 0, height: "100%" }}>
        <ServerGrid
          ...
          emptyIcon="🔄"
          serverSide={true}
          apiUrl={API_ENDPOINTS.TRANSFERS.GET_ALL}
          methodType="GET"
          getAuthHeaders={memoizedGetAuthHeaders}
          defaultSortColumn="transferDate"
          containerWidth="47%"
