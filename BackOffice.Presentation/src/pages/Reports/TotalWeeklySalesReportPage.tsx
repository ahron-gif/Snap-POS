...
interface TotalWeeklySalesReportProps {
  filters?: {
    dateFrom?: string
    dateTo?: string
    storeId?: string
...
interface TotalWeeklySalesRow {
  weekStartDate: string
  total: number
  // Field names mirror the backend response shape (Newtonsoft camelCase of the DTO's
  // Trans / AvgSale properties) so serverSide can render raw rows without remapping.
  trans: number
  avgSale: number
}
...
// Field names match the backend DTO (camelCase from Newtonsoft) so serverSide mode
// can render the raw response directly. Source: TotalWeeklySalesRowDto -> WeekStartDate,
// Total, Trans, AvgSale.
const TOTAL_WEEKLY_SALES_COLUMN_CONFIG = [
  { field: "weekStartDate", headerName: "Week", ratio: 0.3, minWidth: 160 },
  { field: "total", headerName: "Total Amount", ratio: 0.25, minWidth: 160, type: "currency" },
  { field: "trans", headerName: "Trans Count", ratio: 0.2, minWidth: 140, type: "number" },
  { field: "avgSale", headerName: "Avg. Sale", ratio: 0.25, minWidth: 160, type: "currency" },
  { field: "total", headerName: "Avg. Total", ratio: 0.25, minWidth: 160, type: "currency" },
]
...
const TOTAL_WEEKLY_SALES_SCREEN_CODE = "reports.total_weekly_sales"

const TotalWeeklySalesReportPage: React.FC<TotalWeeklySalesReportProps> = ({ filters }) => {
  const { getAuthHeaders } = useAuthHeaders()
  const { currentStore } = useStore()
  const { canExport } = usePermission(TOTAL_WEEKLY_SALES_SCREEN_CODE)

  const todayStr = new Date().toISOString().split("T")[0]
  const defaultDateFrom =
    filters?.dateFrom || new Date(new Date().setDate(new Date().getDate() - 90)).toISOString().split("T")[0]
  ...
  const gridDataRef = useRef<TotalWeeklySalesRow[]>([])
  ...
  const columns: Column[] = useMemo(() => {
    ...
    return TOTAL_WEEKLY_SALES_COLUMN_CONFIG.map((cfg) => {
  ...
  // NOTE: storeId is only included when non-empty (preserves the All-Stores fix); same
  // for departmentId (preserves the Department-filter clearing fix).
  ...
  const handleResponseLoaded = useCallback((responseData: Record<string, unknown>) => {
    ...
    // Backend response carries page-level grand totals (TotalAmount / TotalTransactions);
    // surface them in the header alongside totalRecords (which the grid wires separately).
    const amt = r?.totalAmount ?? r?.TotalAmount
    const trans = r?.totalTransactions ?? r?.TotalTransactions
    setTotalAmount(amt != null ? Number(amt) : 0)
    setTotalTransactions(trans != null ? Number(trans) : 0)
  }, [])

  const handleGridDataChange = useCallback((data: any[]) => {
    gridDataRef.current = data as TotalWeeklySalesRow[]
  }, [])
  ...
  const handleSearch = useCallback(() => {
    ...
    // Pass screen values RAW — empty string for store means "All Stores" and must reach
    // additionalParams' builder so body.storeId stays unset. Same for department.
  ...
  const fetchAllData = useCallback(
    async (overrideFrom?: string, overrideTo?: string): Promise<any[]> => {
      try {
        ...
        const response = await axios.post(API_ENDPOINTS.REPORTS.TOTAL_WEEKLY_SALES, body, { headers })
        ...
        // Keys must match the column `field`s the export modal reads — backend camelCases
        // these as weekStartDate / total / trans / avgSale via Newtonsoft.
        return Array.isArray(dataRaw)
          ? dataRaw.map((r: any) => ({
              weekStartDate: (r.weekStartDate ?? r.WeekStartDate ?? r.date ?? r.Date ?? "").toString(),
              total: Number(r.total ?? r.Total ?? 0),
              trans: Number(r.trans ?? r.Trans ?? r.transactions ?? r.Transactions ?? 0),
              avgSale: Number(r.avgSale ?? r.AvgSale ?? r.averageSale ?? r.AverageSale ?? 0),
            }))
          : []
      } catch (error) {
        console.error("Failed to fetch Total Weekly Sales for export:", error)
        return []
      }
    },
  ...
  const exportModal = useReportExportModal({
    ...
    fetchAllData,
    filename: "total-weekly-sales-report",
    title: "Total Weekly Sales Report",
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Total Weekly Sales</h1>
        ...
        >
          <div className="flex-1 min-h-0">
            <ServerGrid
              ...
              columns={columns}
              apiUrl={API_ENDPOINTS.REPORTS.TOTAL_WEEKLY_SALES}
              serverSide={true}
              methodType="POST"
              ...
              containerWidth="100%"
              gridId="total-weekly-sales-report"
              getRowId={(row) =>
                `${(row as TotalWeeklySalesRow)?.weekStartDate ?? ""}-${(row as TotalWeeklySalesRow)?.total ?? 0}-${
                  (row as TotalWeeklySalesRow)?.trans ?? 0
                }`
              }
            />
          </div>
...
export default TotalWeeklySalesReportPage
