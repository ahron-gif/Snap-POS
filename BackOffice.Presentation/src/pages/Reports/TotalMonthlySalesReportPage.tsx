...
import { useReportAdvancedFilters } from "../../hooks/useReportAdvancedFilters"

interface TotalMonthlySalesReportProps {
  filters?: {
    dateFrom?: string
    dateTo?: string
    storeId?: string
...
interface TotalMonthlySalesRow {
...
const TOTAL_MONTHLY_SALES_COLUMN_CONFIG = [
 // { field: "year", headerName: "Year", ratio: 0.12, minWidth: 80, type: "number" },
  { field: "monthStartDate", headerName: "Month", ratio: 0.18, minWidth: 120 },
  { field: "total", headerName: "Total Amount", ratio: 0.30, minWidth: 160, type: "currency" },
  { field: "transactions", headerName: "Trans Count", ratio: 0.20, minWidth: 140, type: "number" },
    { field: "averageSale", headerName: "Avg. Sale", ratio: 0.20, minWidth: 160, type: "currency" },
    { field: "total", headerName: "Avg Total", ratio: 0.30, minWidth: 160, type: "currency" },
]
...
const TOTAL_MONTHLY_SALES_SCREEN_CODE = "reports.total_monthly_sales"

const TotalMonthlySalesReportPage: React.FC<TotalMonthlySalesReportProps> = ({ filters }) => {
  const { getAuthHeaders } = useAuthHeaders()
  const { currentStore } = useStore()
  const { canExport } = usePermission(TOTAL_MONTHLY_SALES_SCREEN_CODE)

  const todayStr = new Date().toISOString().split("T")[0]
  const defaultDateFrom =
    filters?.dateFrom || new Date(new Date().setFullYear(new Date().getFullYear() - 1)).toISOString().split("T")[0]
  ...
  const {
    ...
    advancedFilterParams, hasActiveAdvancedFilters,
  } = useReportAdvancedFilters()
  const gridDataRef = useRef<TotalMonthlySalesRow[]>([])
  ...
  // Sync with Report Manager filters: update screen + applied state and bump gridKey so
  // ServerGrid refetches with the new filters.
  useEffect(() => {
    if (!filters) return
  ...
  const columns: Column[] = useMemo(() => {
    ...
    return TOTAL_MONTHLY_SALES_COLUMN_CONFIG.map((cfg) => {
      ...
      if (cfg.field === "monthStartDate") {
        ;(col as Column & { cellRenderer?: (v: string) => string }).cellRenderer = (value: string) =>
          formatDateDMY(value)
      }
      return col
    })
  ...
  // NOTE: storeId / departmentId are only added when non-empty — empty string means
  // "All Stores" / "All Departments" and must NOT be sent so the server-side filter clears.
  const additionalParams = useMemo(() => {
    ...
    if (appliedDepartmentId && appliedDepartmentId.trim().length > 0) {
      params.departmentId = appliedDepartmentId.trim()
    }
  ...
  const handleResponseLoaded = useCallback((responseData: Record<string, unknown>) => {
    ...
    // Page-level totals (sum across the entire filtered set, not just the current page)
    // are returned by the backend on each request — pluck them with dual-casing fallback.
    setTotalAmount(Number(r?.totalAmount ?? r?.TotalAmount ?? 0))
    setTotalTransactions(Number(r?.totalTransactions ?? r?.TotalTransactions ?? 0))
  }, [])

  const handleGridDataChange = useCallback((data: any[]) => {
    gridDataRef.current = data as TotalMonthlySalesRow[]
  }, [])
  ...
  const handleSearch = useCallback(() => {
    ...
    // Pass screenStoreId / screenDepartmentId RAW — empty string means "All Stores" /
    // "All Departments" and must propagate so additionalParams omits the key from the
    // body and the server-side filter clears.
    setAppliedStoreId(screenStoreId)
    setAppliedDepartmentId(screenDepartmentId)
    setGridKey((k) => k + 1)
  }, [dateFrom, dateTo, screenStoreId, screenDepartmentId])

  // Fetch every row for export/print, ignoring pagination by asking for one giant page.
  const fetchAllData = useCallback(
    async (overrideFrom?: string, overrideTo?: string): Promise<any[]> => {
      try {
        ...
        const response = await axios.post(API_ENDPOINTS.REPORTS.TOTAL_MONTHLY_SALES, body, { headers })
        ...
        return Array.isArray(dataRaw)
          ? dataRaw.map((r: any) => {
              const monthStart =
                (r.monthStartDate ?? r.MonthStartDate) != null
                  ? new Date(r.monthStartDate ?? r.MonthStartDate)
                  : null
              const year =
                r.year ?? r.Year ??
                (monthStart instanceof Date && !Number.isNaN(monthStart.getTime()) ? monthStart.getFullYear() : 0)
              const month =
                r.month ?? r.Month ??
                (monthStart instanceof Date && !Number.isNaN(monthStart.getTime()) ? monthStart.getMonth() + 1 : 0)
              return {
                monthStartDate:
                  monthStart instanceof Date && !Number.isNaN(monthStart.getTime())
                    ? monthStart.toISOString()
                    : (r.monthStartDate ?? r.MonthStartDate ?? "").toString(),
                year,
                month,
        ...
        console.error("Failed to fetch Total Monthly Sales for export:", error)
        return []
      }
    },
  ...
  const exportModal = useReportExportModal({
    ...
    fetchAllData,
    filename: "total-monthly-sales-report",
    title: "Total Monthly Sales Report",
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Total Monthly Sales</h1>
        ...
        >
          <div className="flex-1 min-h-0">
            <ServerGrid
              ...
              columns={columns}
              apiUrl={API_ENDPOINTS.REPORTS.TOTAL_MONTHLY_SALES}
              serverSide={true}
              methodType="POST"
              ...
              containerWidth="100%"
              gridId="total-monthly-sales-report"
              getRowId={(row) =>
                `${(row as TotalMonthlySalesRow)?.year ?? 0}-${(row as TotalMonthlySalesRow)?.month ?? 0}-${
                  (row as TotalMonthlySalesRow)?.total ?? 0
                }`
              }
            />
          </div>
...
export default TotalMonthlySalesReportPage
