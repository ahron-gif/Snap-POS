...
/**
 * Department Daily Sales — pivot view.
 *
 * Mirrors the desktop's `RepDepartmentDailySales` DevExpress PivotGrid. Important: the row /
 * column orientation is INVERTED from the Item pivots:
 *
 *   Item pivots:       rows = (Department, Item, Barcode),  columns = dates
 *   Department pivot:  rows = (Date, Store),                columns = departments
 *
 * Cells under each department column = Amount + Qty (toggle via the Show selector).
 *
 * Double-click on a row opens an Item Daily Sales tab scoped to the (date, store) so the user
 * can drill into which items drove that day's department totals — mirrors how the desktop's
 * cell double-click opens RepItemsDailySales.
 */

// ---------- types -----------------------------------------------------------

interface DepartmentDailySalesPivotProps {
  filters?: {
    dateFrom?: string
    dateTo?: string
    storeId?: string
...
interface ApiColumn { name: string; id?: string | null }
interface ApiRow {
  date: string                      // yyyy-MM-dd
  storeId?: string | null
  storeName: string
  cells: Record<string, ApiCell>    // keyed by department NAME
}
interface ApiTotals {
  byDepartment: Record<string, ApiCell>
  grand: ApiCell
}
interface ApiPivotResponse {
  departments: ApiColumn[]
  rows: ApiRow[]
  totals: ApiTotals
  totalRecords: number
}
...
const REPORT_TITLE = "Department Daily Sales"
const SCREEN_CODE = "reports.department_daily_sales"
...
const DepartmentDailySalesPivotPage: React.FC<DepartmentDailySalesPivotProps> = ({ filters }) => {
  ...
  // Sort direction for the right-side Department columns (matches desktop's column header
  // sort indicator).
  const [deptSortDir, setDeptSortDir] = useState<"asc" | "desc">("asc")
  // Date order on the left side — newest first by default (matches desktop's "DayOfYear DESC").
  const [dateSortDir, setDateSortDir] = useState<"asc" | "desc">("desc")
  // Show selector — Amount only / Qty only / Both. Matches the weekly+monthly Item pivots.
  ...
  const fetchData = useCallback(async (overrides?: {
    dateFrom?: string; dateTo?: string; storeId?: string;
  }) => {
    ...
    try {
      ...
      }>(API_ENDPOINTS.REPORTS.DEPARTMENT_DAILY_SALES_PIVOT, body, { headers })
  ...
  // Left columns: Date + Store. Both pinned, neither is grouped — rows are flat.
  ...
  // Right groups: one per department, ordered per the toggle. Sub-columns reflect Show mode.
  const orderedDepartments = useMemo(() => {
    const ds = data?.departments ?? []
    const sorted = [...ds].sort((a, b) =>
      a.name.localeCompare(b.name, undefined, { sensitivity: "base" })
    )
    return deptSortDir === "desc" ? sorted.reverse() : sorted
  }, [data?.departments, deptSortDir])
  ...
  const rightGroups: PivotRightGroup[] = useMemo(() => {
    ...
    return orderedDepartments.map((d) => ({
      key: d.name,
      header: d.name,
      subs,
    }))
  }, [orderedDepartments, valueMode])

  // Rows: sort by date according to dateSortDir, then by store name.
  const orderedRows = useMemo(() => {
    const rs = data?.rows ?? []
    return [...rs].sort((a, b) => {
      const dc = a.date.localeCompare(b.date)
      const byDate = dateSortDir === "asc" ? dc : -dc
      if (byDate !== 0) return byDate
      return a.storeName.localeCompare(b.storeName, undefined, { sensitivity: "base" })
    })
  }, [data?.rows, dateSortDir])
  ...
  const renderGrandTotal = useCallback(
    (rg: PivotRightGroup, sub: PivotRightGroup["subs"][number]) => {
      const c = data?.totals?.byDepartment?.[rg.key]
      if (!c) return ""
      return sub.key === "amount" ? fmtMoney(c.amount) : fmtQty(c.qty)
    }, [data])

  /**
   * Row-level double-click (left-side / Date or Store cell) — opens Item Daily Sales scoped
   * to the (Date, Store) but NOT to a specific department. Useful when you want to see all
   * items the store sold on that day regardless of department.
   */
  const onRowDoubleClick = useCallback((row: ApiRow) => {
    const dateKey = row.date
    const storeId = (row.storeId ?? "").toString()
    const storeName = row.storeName || appliedStoreName
    openTab({
      id: `item-daily-sales-from-dept-${dateKey}-${storeId || "all"}-all`,
      title: `Item Daily Sales — ${fmtDate(dateKey)}`,
      component: "ItemDailySalesPivotPage",
      props: {
        filters: {
          dateFrom: dateKey,
          dateTo: dateKey,
  ...
  /**
   * Cell-level double-click (a specific department cell on the right) — passes BOTH the
   * (Date, Store) from the row AND the Department from the clicked column. Mirrors desktop
   * RepDepartmentDailySales → RepItemsDailySales which sets Connector.MemoesTag.Department
   * from the clicked cell's column.
   */
  const onCellDoubleClick = useCallback(
    (row: ApiRow, rg: PivotRightGroup, _sub: PivotRightGroup["subs"][number]) => {
      ...
      const storeId  = (row.storeId ?? "").toString()
      const storeName = row.storeName || appliedStoreName
      const deptName = String(rg.key)
      // Look up the canonical DepartmentID from the departments[] catalog the API returned.
      // The pivot row's cells don't carry the ID — only the name — so we resolve it once
      // here. Department names without an ID (e.g. "[NO DEPARTMENT]") pass through as
      // departmentId=undefined so the next report shows all items in that bucket.
      const deptId = data?.departments?.find((d) => d.name === deptName)?.id ?? undefined

      openTab({
        id: `item-daily-sales-from-dept-${dateKey}-${storeId || "all"}-${deptId ?? deptName.toLowerCase().replace(/\s+/g, "_")}`,
        title: `Item Daily Sales — ${fmtDate(dateKey)} · ${deptName}`,
        component: "ItemDailySalesPivotPage",
        props: {
          filters: {
            dateFrom: dateKey,
    ...
    [openTab, appliedStoreName, data?.departments]
  )

  const grand = data?.totals?.grand ?? { qty: 0, amount: 0 }
  const displayStoreName = appliedStoreId ? appliedStoreName : "All Stores"

  // ---- Export modal (no date filter — same trick as the item pivots) ----

  const amtField = (name: string) => `amt_${name}`
  const qtyField = (name: string) => `qty_${name}`

  const exportColumns = useMemo<GridUtilsColumn[]>(() => {
    const cols: GridUtilsColumn[] = [
      { field: "dateDisplay", headerName: "Date",  width: 110, dataType: "string" },
  ...
  const fetchAllExportRows = useCallback(async (): Promise<any[]> => {
    ...
    for (const r of orderedRows) {
      const fr: Record<string, any> = {
        dateDisplay: fmtDate(r.date),
        storeName: r.storeName || "",
      }
      for (const d of orderedDepartments) {
        const c = r.cells?.[d.name]
        fr[amtField(d.name)] = c ? c.amount : null
        fr[qtyField(d.name)] = c ? c.qty : null
      }
      flat.push(fr)
    }
    // Grand total row
    const gr: Record<string, any> = { dateDisplay: "Grand Total", storeName: "" }
    for (const d of orderedDepartments) {
      const c = data.totals?.byDepartment?.[d.name]
      gr[amtField(d.name)] = c ? c.amount : null
      gr[qtyField(d.name)] = c ? c.qty : null
    }
    flat.push(gr)
    return flat
  }, [data, orderedRows, orderedDepartments])

  const exportModal = useExportModal({
    columns: exportColumns,
    fetchAllData: fetchAllExportRows,
    filename: "department-daily-sales",
    pdfOptions: {
      ...
      // Keep Date / Store on every horizontal page so a reader scanning across many
      // department columns can always tell which (date, store) row a cell belongs to.
      repeatColumns: 2,
    ...
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="space-y-1">
              ...
              <button
                ...
                title={dateSortDir === "asc" ? "Oldest first — click to reverse" : "Latest first — click to reverse"}
            ...
            <div className="space-y-1">
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Department Order</label>
              <button
                type="button"
                onClick={() => setDeptSortDir((d) => (d === "asc" ? "desc" : "asc"))}
                title={deptSortDir === "asc" ? "A → Z — click to reverse" : "Z → A — click to reverse"}
                className="h-10 px-3 text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors flex items-center gap-2"
              >
                {deptSortDir === "asc"
      ...
      <div className="flex-1 min-h-0 p-6">
        <StickyPivotTable<ApiRow>
          ...
          rows={orderedRows}
          // No groupBy — rows are flat (one per Date + Store).
...
export default DepartmentDailySalesPivotPage
