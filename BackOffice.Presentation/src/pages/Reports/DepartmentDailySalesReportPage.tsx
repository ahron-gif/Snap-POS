...
interface DepartmentDailySalesReportProps {
  filters?: {
    dateFrom?: string
    dateTo?: string
    storeId?: string
...
interface DepartmentDailySalesRow {
  saleDate: string
  storeName: string
  department: string
  qty: number
...
const DEPARTMENT_DAILY_SALES_SCREEN_CODE = "reports.department_daily_sales"

const DepartmentDailySalesReportPage: React.FC<DepartmentDailySalesReportProps> = ({ filters }) => {
  const { getAuthHeaders } = useAuthHeaders()
  const { currentStore } = useStore()
  const { canExport } = usePermission(DEPARTMENT_DAILY_SALES_SCREEN_CODE)
  ...
  const [rows, setRows] = useState<DepartmentDailySalesRow[]>([])
  ...
  const columns: Column[] = useMemo(() => {
    ...
    return DEPARTMENT_DAILY_SALES_COLUMN_CONFIG.map((cfg) => {
  ...
  const fetchData = useCallback(
    async (overrides?: { dateFrom?: string; dateTo?: string; storeId?: string }) => {
      ...
      try {
        ...
        const response = await axios.post(API_ENDPOINTS.REPORTS.DEPARTMENT_DAILY_SALES, body, { headers })
        const ok = response.data?.isSuccess ?? response.data?.IsSuccess

        if (!ok) {
          const message =
            response.data?.message || response.data?.Message || "Failed to load Department Daily Sales report"
          setError(message)
          setRows([])
          setTotalRecords(0)
          setTotalQty(0)
        ...
        const list: DepartmentDailySalesRow[] = Array.isArray(dataRaw)
          ? dataRaw.map((r: any) => ({
        ...
        console.error("Error loading Department Daily Sales report", e)
        const data = e?.response?.data
        const serverMessage = data?.message ?? data?.Message
        setError(serverMessage || e?.message || "Failed to load Department Daily Sales report")
        setRows([])
        setTotalRecords(0)
        setTotalQty(0)
        setTotalAmount(0)
  ...
  const fetchAllData = useCallback(
    async (overrideFrom?: string, overrideTo?: string): Promise<any[]> => {
      try {
        ...
        console.error("Failed to fetch Department Daily Sales for export:", error)
        return []
      }
    },
  ...
  const exportModal = useReportExportModal({
    ...
    fetchAllData,
    filename: "department-daily-sales-report",
    title: "Department Daily Sales Report",
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Department Daily Sales</h1>
        ...
        >
          <div className="flex-1 min-h-0">
            <ServerGrid
              ...
              getRowId={(row) =>
                `${(row as DepartmentDailySalesRow)?.saleDate ?? ""}-${(row as DepartmentDailySalesRow)?.storeName ?? ""}-${(row as DepartmentDailySalesRow)?.department ?? ""}`
              }
            />
          </div>
        </div>
...
export default DepartmentDailySalesReportPage
