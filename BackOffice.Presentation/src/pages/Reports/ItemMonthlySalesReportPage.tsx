...
interface ItemMonthlySalesReportProps {
  filters?: {
    dateFrom?: string
    dateTo?: string
    storeId?: string
...
const ITEM_MONTHLY_SALES_COLUMN_CONFIG = [
  { field: "year", headerName: "Year", ratio: 0.08, minWidth: 90, type: "number" },
  { field: "monthName", headerName: "Month", ratio: 0.12, minWidth: 110 },
  { field: "department", headerName: "Department", ratio: 0.18, minWidth: 160 },
  { field: "itemName", headerName: "Item Name", ratio: 0.24, minWidth: 220 },
  { field: "barcodeNumber", headerName: "Barcode", ratio: 0.14, minWidth: 140 },
  { field: "qty", headerName: "Qty", ratio: 0.08, minWidth: 90, type: "number" },
  { field: "total", headerName: "Amount", ratio: 0.09, minWidth: 120, type: "currency", visible: false },
  { field: "averagePrice", headerName: "Avg. Price", ratio: 0.07, minWidth: 110, type: "currency" },
]
...
const ITEM_MONTHLY_SALES_SCREEN_CODE = "reports.item_monthly_sales"

const ItemMonthlySalesReportPage: React.FC<ItemMonthlySalesReportProps> = ({ filters }) => {
  const { getAuthHeaders } = useAuthHeaders()
  const { currentStore } = useStore()
  const { canExport } = usePermission(ITEM_MONTHLY_SALES_SCREEN_CODE)
  ...
  const gridDataRef = useRef<ItemMonthlySalesRow[]>([])
  ...
  // Sync with Report Manager filters — bump gridKey so ServerGrid refetches with new params.
  useEffect(() => {
    if (!filters) return
  ...
  const columns: Column[] = useMemo(() => {
    ...
    return ITEM_MONTHLY_SALES_COLUMN_CONFIG.map((cfg) => {
      ...
      // Respect visibility flag from config (used to hide Amount from UI but keep for totals/exports)
      if ((cfg as any).visible === false) {
        ;(col as Column & { visible?: boolean }).visible = false
      }
  ...
  const handleGridDataChange = useCallback((data: any[]) => {
    gridDataRef.current = data as ItemMonthlySalesRow[]
  }, [])
  ...
  const fetchAllData = useCallback(
    async (overrideFrom?: string, overrideTo?: string): Promise<any[]> => {
      try {
        ...
        const response = await axios.post(API_ENDPOINTS.REPORTS.ITEM_MONTHLY_SALES, body, { headers })
        ...
        return Array.isArray(dataRaw)
          ? dataRaw.map((r: any) => {
              ...
              const year =
                r.year ??
                r.Year ??
                (monthStart instanceof Date && !isNaN(monthStart.getTime()) ? monthStart.getFullYear() : undefined)
              const monthNameRaw = r.monthName ?? r.MonthName
              const monthName =
                monthNameRaw && typeof monthNameRaw === "string" && monthNameRaw.trim().length > 0
                  ? monthNameRaw
                  : monthStart instanceof Date && !isNaN(monthStart.getTime())
        ...
        console.error("Failed to fetch Item Monthly Sales for export:", error)
        return []
      }
    },
  ...
  const exportModal = useReportExportModal({
    ...
    fetchAllData,
    filename: "item-monthly-sales-report",
    title: "Item Monthly Sales Report",
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Item Monthly Sales</h1>
        ...
        >
          <div className="flex-1 min-h-0">
            <ServerGrid
              ...
              columns={columns}
              apiUrl={API_ENDPOINTS.REPORTS.ITEM_MONTHLY_SALES}
              serverSide={true}
              methodType="POST"
              ...
              containerWidth="100%"
              gridId="item-monthly-sales-report"
              getRowId={(row) =>
                `${(row as ItemMonthlySalesRow)?.monthStartDate ?? ""}-${(row as ItemMonthlySalesRow)?.itemName ?? ""}-${(row as ItemMonthlySalesRow)?.barcodeNumber ?? ""}`
              }
            />
          </div>
        </div>
...
export default ItemMonthlySalesReportPage
