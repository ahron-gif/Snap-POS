...
interface DepartmentWeeklySalesReportProps {
  filters?: {
    dateFrom?: string
    dateTo?: string
    storeId?: string
...
interface DepartmentWeeklySalesRow {
...
const DEPARTMENT_WEEKLY_SALES_COLUMN_CONFIG = [
  { field: "storeName", headerName: "Store", ratio: 0.25, minWidth: 200 },
  { field: "department", headerName: "Department", ratio: 0.35, minWidth: 220 },
  { field: "weekStartDate", headerName: "Week", ratio: 0.16, minWidth: 140 },
  { field: "qty", headerName: "Qty", ratio: 0.12, minWidth: 100, type: "number" },
  { field: "total", headerName: "Amount", ratio: 0.12, minWidth: 140, type: "currency" },
]
...
const DEPARTMENT_WEEKLY_SALES_SCREEN_CODE = "reports.department_weekly_sales"

const DepartmentWeeklySalesReportPage: React.FC<DepartmentWeeklySalesReportProps> = ({ filters }) => {
  const { getAuthHeaders } = useAuthHeaders()
  const { currentStore } = useStore()
  const { canExport } = usePermission(DEPARTMENT_WEEKLY_SALES_SCREEN_CODE)
  ...
  const [rows, setRows] = useState<DepartmentWeeklySalesRow[]>([])
  ...
  const columns: Column[] = useMemo(() => {
    ...
    return DEPARTMENT_WEEKLY_SALES_COLUMN_CONFIG.map((cfg) => {
  ...
  const fetchData = useCallback(
    async (overrides?: { dateFrom?: string; dateTo?: string; storeId?: string }) => {
      ...
      try {
        ...
        const response = await axios.post(API_ENDPOINTS.REPORTS.DEPARTMENT_WEEKLY_SALES, body, { headers })
        const ok = response.data?.isSuccess ?? response.data?.IsSuccess

        if (!ok) {
          const message =
            response.data?.message || response.data?.Message || "Failed to load Department Weekly Sales report"
          setError(message)
          setRows([])
          setTotalRecords(0)
          setTotalQty(0)
        ...
        const list: DepartmentWeeklySalesRow[] = Array.isArray(dataRaw)
          ? dataRaw.map((r: any) => ({
        ...
        console.error("Error loading Department Weekly Sales report", e)
        const data = e?.response?.data
        const serverMessage = data?.message ?? data?.Message
        setError(serverMessage || e?.message || "Failed to load Department Weekly Sales report")
        setRows([])
        setTotalRecords(0)
        setTotalQty(0)
        setTotalAmount(0)
  ...
  const fetchAllData = useCallback(
    async (overrideFrom?: string, overrideTo?: string): Promise<any[]> => {
      try {
        ...
        console.error("Failed to fetch Department Weekly Sales for export:", error)
        return []
      }
    },
  ...
  const exportModal = useReportExportModal({
    ...
    fetchAllData,
    filename: "department-weekly-sales-report",
    title: "Department Weekly Sales Report",
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Department Weekly Sales</h1>
        ...
        >
          <div className="flex-1 min-h-0">
            <ServerGrid
              ...
              getRowId={(row) =>
                `${(row as DepartmentWeeklySalesRow)?.weekStartDate ?? ""}-${(row as DepartmentWeeklySalesRow)?.storeName ?? ""}-${(row as DepartmentWeeklySalesRow)?.department ?? ""}`
              }
            />
          </div>
        </div>
...
export default DepartmentWeeklySalesReportPage
