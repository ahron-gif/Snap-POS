...
interface ItemWeeklySalesReportProps {
  filters?: {
    dateFrom?: string
    dateTo?: string
    storeId?: string
...
const ITEM_WEEKLY_SALES_COLUMN_CONFIG = [
  { field: "department", headerName: "Department", ratio: 0.22, minWidth: 160 },
  { field: "itemName", headerName: "Item Name", ratio: 0.34, minWidth: 220 },
  { field: "barcodeNumber", headerName: "Barcode", ratio: 0.18, minWidth: 160 },
  { field: "weekStartDate", headerName: "Week", ratio: 0.14, minWidth: 120 },
  { field: "qty", headerName: "Qty", ratio: 0.12, minWidth: 90, type: "number" },
  // Amount (total) is kept for totals/export but hidden from the visible grid.
  { field: "total", headerName: "Amount", ratio: 0.0, minWidth: 0, type: "currency", visible: false },
  { field: "averagePrice", headerName: "Avg. Price", ratio: 0.0, minWidth: 0, type: "currency" },
]
...
const ITEM_WEEKLY_SALES_SCREEN_CODE = "reports.item_weekly_sales"

const ItemWeeklySalesReportPage: React.FC<ItemWeeklySalesReportProps> = ({ filters }) => {
  const { getAuthHeaders } = useAuthHeaders()
  const { currentStore } = useStore()
  const { canExport } = usePermission(ITEM_WEEKLY_SALES_SCREEN_CODE)
  ...
  const gridDataRef = useRef<ItemWeeklySalesRow[]>([])
  ...
  // Sync with Report Manager filters — bump gridKey to trigger a refetch via serverSide grid.
  useEffect(() => {
    if (!filters) return
  ...
  const columns: Column[] = useMemo(() => {
    ...
    return ITEM_WEEKLY_SALES_COLUMN_CONFIG.map((cfg) => {
      ...
      if (cfg.type === "number") {
        col.cellRenderer = (value: any) => numberFmt(value)
      }
      if (cfg.field === "weekStartDate") {
        col.cellRenderer = (value: any) => formatDateDMY(value)
      }
      return col
    })
  }, [gridContainerWidth])
  ...
  // and just varies startRow/endRow; only handleSearch (or a filters prop change) mutates it.
  ...
  const handleGridDataChange = useCallback((data: any[]) => {
    gridDataRef.current = data as ItemWeeklySalesRow[]
  }, [])
  ...
  const fetchAllData = useCallback(
    async (overrideFrom?: string, overrideTo?: string): Promise<any[]> => {
      try {
        ...
        const response = await axios.post(API_ENDPOINTS.REPORTS.ITEM_WEEKLY_SALES, body, { headers })
        ...
        return Array.isArray(dataRaw)
          ? dataRaw.map((r: any) => ({
              weekStartDate: (r.weekStartDate ?? r.WeekStartDate ?? "").toString(),
        ...
        console.error("Failed to fetch Item Weekly Sales for export:", error)
        return []
      }
    },
  ...
  const exportModal = useReportExportModal({
    ...
    fetchAllData,
    filename: "item-weekly-sales-report",
    title: "Item Weekly Sales Report",
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Item Weekly Sales</h1>
        ...
        >
          <div className="flex-1 min-h-0">
            <ServerGrid
              ...
              columns={columns}
              apiUrl={API_ENDPOINTS.REPORTS.ITEM_WEEKLY_SALES}
              serverSide={true}
              methodType="POST"
              ...
              containerWidth="100%"
              gridId="item-weekly-sales-report"
              getRowId={(row) =>
                `${(row as ItemWeeklySalesRow)?.weekStartDate ?? ""}-${(row as ItemWeeklySalesRow)?.itemName ?? ""}-${(row as ItemWeeklySalesRow)?.barcodeNumber ?? ""}`
              }
            />
          </div>
        </div>
...
export default ItemWeeklySalesReportPage
