...
// Row shape returned by the paged endpoint (camelCase LegacyGroupPreviewDto)
interface LegacyGroupPreviewRow {
  groupId: string;
  groupName: string;
  code: string;
  isSystem: boolean;
...
const GROUP_IMPORT_GRID_ID = 'group-import-legacy-grid';

// ── Cell renderers ───────────────────────────────────────────────
const statusCell = (_v: unknown, row?: LegacyGroupPreviewRow): React.ReactNode => {
  if (!row) return null;
...
const activeCell = (_v: unknown, row?: LegacyGroupPreviewRow): React.ReactNode => {
  if (!row) return null;
  const base = 'px-2 py-0.5 rounded-full text-xs font-medium';
  return row.isActive
    ? <span className={`${base} bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-300`}>Active</span>
    : <span className={`${base} bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300`}>Inactive</span>;
};

const warningsCell = (_v: unknown, row?: LegacyGroupPreviewRow): React.ReactNode => {
...
const columnDefs: GridColDef[] = [
  { field: 'groupName', headerName: 'Group Name', width: 240, type: 'string', sortable: true, filterable: false },
  { field: 'code', headerName: 'Role Code', width: 200, type: 'string', sortable: true, filterable: false },
  { field: 'isActive', headerName: 'Active', width: 110, type: 'string', sortable: true, filterable: false, cellRenderer: activeCell },
  { field: 'alreadyImported', headerName: 'Status', width: 160, type: 'string', sortable: true, filterable: false, cellRenderer: statusCell },
  { field: 'warnings', headerName: 'Warnings', width: 130, type: 'string', sortable: false, filterable: false, cellRenderer: warningsCell },
];
...
const GroupImportPage: React.FC = () => {
  ...
  const runImport = async (importAll: boolean) => {
    ...
    const groupIds = importAll ? null : Array.from(selectedRows);
    if (!importAll && groupIds!.length === 0) return;
    ...
    try {
      const res = await fetch(API_ENDPOINTS.GROUP_IMPORT.IMPORT, {
        ...
        body: JSON.stringify({ groupIds, overwriteExisting: overwrite }),
      });
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Import Groups</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Migrate a tenant's desktop Back Office user groups into the web platform as RBAC roles.
          The original groups are left untouched, and groups already imported are never duplicated.
        </p>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        {selectedTenantId ? (
          <ServerGrid
            key={`group-import-${selectedTenantId}-${remountKey}`}
            data={[]}
            columns={columns}
            gridId={GROUP_IMPORT_GRID_ID}
            loading={false}
            error={null}
            pagination={true}
            pageSize={10}
            serverSide={true}
            apiUrl={API_ENDPOINTS.GROUP_IMPORT.GET_LEGACY_GROUPS_PAGED}
            methodType="GET"
            getAuthHeaders={getGridHeaders}
            defaultSortColumn="groupName"
            defaultSortDirection="asc"
            title="Legacy Groups"
            emptyMessage="No legacy groups found for this tenant"
            emptyIcon="👥"
            showCheckboxes={true}
          ...
          <div className="h-full flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm">
              Choose a tenant from the dropdown above to view its legacy Back Office user groups and
              import them as RBAC roles.
            </p>
          </div>
        )}
      </div>
